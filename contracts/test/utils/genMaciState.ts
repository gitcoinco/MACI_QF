...
export const genMaciStateFromContract = async (
  ...
  for (let i = fromBlock; i <= lastBlock; i += blocksPerRequest + 1) {
    ...
    deployPollLogs.forEach((event: any) => {
      assert(!!event);
  ...
  assert(
    foundPollIds.has(Number(pollId)),
  ...
  const [
    ...
    pollContract
      ...
      .then((values: any) => values.map(Number)),
  ...
  const batchSizes = {
    ...
    messageBatchSize:
      MESSAGE_TREE_ARITY ** Number(onChainTreeDepths.messageTreeSubDepth),
  };
  ...
  for (let i = fromBlock; i <= lastBlock; i += blocksPerRequest + 1) {
    ...
    publishMessageLogs.forEach((event: any) => {
      ...
      const message = new Message(
        ...
        event.args._message[1].map((x: any) => BigInt(x))
      );
    ...
    mergeMessageAqSubRootsLogs.forEach((event: any) => {
      assert(!!event);
    ...
    mergeMessageAqLogs.forEach((event: any) => {
      assert(!!event);
